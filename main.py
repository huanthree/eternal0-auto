...
def add_server_time(server_url="https://gpanel.eternalzero.cloud/server/5302206f"):
    ...
    """
    # 获取环境变量
    # 将 PTERODACTYL_SESSION 替换为 REMEMBER_WEB_COOKIE
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if remember_web_cookie:
                ...
                # Cookie 名称更改为 'remember_web_59ba36addc2b2f9401580f014c7f58ea4e30989d'
                # 域名需要设置为 .eternalzero.cloud 以覆盖所有子域名
                session_cookie = Cookie(
                    name='remember_web_59ba36addc2b2f9401580f014c7f58ea4e30989d', # 修改这里的 Cookie 名称
                    value=remember_web_cookie,
                    domain='.eternalzero.cloud',
                    path='/',
                    expires=time.time() + 3600 * 24 * 365, # 延长过期时间，例如一年，以匹配 "remember me" 的持久性
                    httpOnly=True,
                    secure=True,
                    sameSite='Lax'
                )
                ...
                page.goto(server_url, wait_until="networkidle", timeout=60000) # 增加超时到 60 秒
                ...
                if "login" in page.url or "auth" in page.url:
                    ...
                    remember_web_cookie = None # 标记为失效，强制回退到密码登录
                else:
            ...
            if not remember_web_cookie:
                ...
                # 增加 goto 的超时时间
                page.goto(login_url, wait_until="networkidle", timeout=60000) # 增加超时到 60 秒
                ...
                try:
                    ...
                    page.wait_for_url(server_url, timeout=60000) # 增加等待 URL 的超时
